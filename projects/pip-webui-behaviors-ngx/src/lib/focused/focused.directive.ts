...
export class PipFocusedDirective implements AfterViewInit {
  ...
  constructor(private elRef: ElementRef, private renderer: Renderer2) {
    __ngRendererSetElementAttributeHelper(renderer, this.elRef.nativeElement, 'tabindex', '1');
    ...
    for (let i = 0; i < this.controlsLength; i++) {
      ...
      element.addEventListener('focus', (event) => {
        ...
        if (!this._focusedOpacity) {
          ...
          if (this._focusedClass) {
            element.classList.add(this._focusedClass);
          }
        } else {
          if (this._focusedClass) {
            element.classList.add(this._focusedClass);
          }
          if (this.focusedOpacityClass) {
            element.classList.add(this.focusedOpacityClass);
          }
        }
      });
      ...
      element.addEventListener('focusout', (event) => {
        ...
        if (!this._focusedOpacity) {
          ...
          if (this._focusedClass) {
            element.classList.remove(this._focusedClass);
          }
        } else {
          if (this._focusedClass) {
            element.classList.remove(this._focusedClass);
          ...
          if (this.focusedOpacityClass) {
            element.classList.remove(this.focusedOpacityClass);
          }
        }
      });
    }
  ...
  private keydownListener(e) {
    ...
    if (
      ...
      const increment = keyCode === KeyCode.RIGHT_ARROW || keyCode === KeyCode.DOWN_ARROW ? 1 : -1,
      ...
      setTimeout(() => {
        ...
        if (index < this.controls.length) {
          ...
          while (index >= this.controls.length && index > -1) {
            index--;
          }
          this.controls[index].focus();
        }
  ...
  private getElementIndex(elements) {
    for (let i = 0; i < elements.length; i++) {
      if (elements[i].classList.contains(this._focusedClass)) {
        return i;
      }
    }
