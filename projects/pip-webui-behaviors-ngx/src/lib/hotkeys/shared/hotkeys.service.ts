...
export class PipHotkeysService implements OnDestroy {
  ...
  constructor(@Optional() @Inject(HOTKEYS_DEFAULT_OPTIONS) options: HotkeyOptions, private router: Router) {
    if (options) {
  ...
  ngOnDestroy() {
    for (const key of Object.keys(this._mti)) {
      if (!this._mti.hasOwnProperty(key)) {
        continue;
      }
      this._mti[key].reset();
    }
